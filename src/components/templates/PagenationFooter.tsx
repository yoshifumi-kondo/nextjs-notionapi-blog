...
const PagenationFooter: FC<PagenationFooterProps> = ({ pageLength = 23, position = 21 }) => {
  const sampleArray = Array.from(new Array(pageLength)).map((_v, i) => i + 1);
  const hooterArray: Array<'...' | number> =
    pageLength > 10 && (3 > position || position > pageLength - 3)
  ...
  return (
    <div className='flex md:gap-5 gap-1 flex-wrap justify-center items-center'>
      ...
      {hooterArray.map((v, i) => {
        ...
        if (
          (3 > position || position > pageLength - 3) &&
          (v === 3 || v === pageLength - 2) &&
          pageLength > 4
        ) {
          return (
